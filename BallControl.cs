...
public class BallControl : MonoBehaviour {
	...
	void Update () {
        ...
        if (isOnClick)
        {
            ...
            // 공을 0,0으로 보내려면 x값과 z값이 얼마인지 구함
            float tempX = zeroPosition(tr.position.x);
            float tempZ = zeroPosition(tr.position.z);

            // 0,0으로 보냈을때 레이의 좌표를 구함
            Vector3 zeroRay = new Vector3(rayPositionInfo.point.x + tempX, 0.0f, rayPositionInfo.point.z + tempZ);
            // 이때 피타고라스 정리로 0,0에서 레이 좌표까지 길이를 구함(이것이 힘의 크기)
            float zeroTolength = Mathf.Sqrt(Mathf.Pow(zeroRay.x, 2) + Mathf.Pow(zeroRay.z, 2));
            // 힘의 크기 저장
            power = zeroTolength;
            ...
            //Debug.Log(zeroRay);
            //Debug.Log(zeroTolength);
            //Debug.Log(sin);
            //Debug.Log(ArrowTr.rotation);
            //Debug.Log(degrees);
            //Debug.Log(Quaternion.Euler(90.0f, 0.0f, 0.0f));
            // 각도에 맞게 회전
            ArrowTr.rotation = Quaternion.Euler(90.0f, 0.0f, degrees);
            //
            ArrowTr.localScale = new Vector3(zeroTolength * 2, 1.0f, 1.0f);
        ...
        if(Input.GetMouseButtonDown(0))
        {
            ...
            if (Physics.Raycast(ray, out rayInfo, Mathf.Infinity))
            {
                ...
                if (rayInfo.transform.tag == "Floow" && isOnClick)
                {
